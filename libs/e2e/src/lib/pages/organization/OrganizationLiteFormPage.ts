...
export default class OrganizationLiteFormPage {
  ...
  public fillOrganizationInformation(org: Partial<Organization> = ORGANIZATION) {
    ...
    cy.get('static-select input[test-id="address-country"]').click();
  ...
  public fillAllExceptOne(org: Partial<Organization>, key) {
    switch (key) {
      case 'denomination' :
        cy.get('static-select input[test-id="address-country"]').should('not.exist');
  ...
  public verifyInformation(org: Partial<Organization>, role: string) {
    ...
    cy.get('static-select input[test-id="address-country"]').should('contain.value', org.address.country);
    cy.get('static-select input[test-id="address-country"]').should('be.disabled');
    if (role === "seller") {
      cy.get('organization-lite-form mat-button-toggle-group[test-id="appAccessToggleGroup"]')
