...
import { Language, Media, Territory, Timestamp, LanguageRecord, Duration, Negotiation } from '@blockframes/shared/model';
...
function languageDiff(
  before: LanguageRecord,
  after: LanguageRecord
): ArrayDifferences<LanguageRecord> {
  const addedLanguages = Object.keys(after ?? {}).filter(
    (key) => !Object.keys(before).includes(key)
  ) as Language[];
  const removedLanguages = Object.keys(before).filter(
    (key) => !Object.keys(after ?? {}).includes(key)
  ) as Language[];
...
function negotiationDiff(before: Negotiation, after: Negotiation): NegotiationDifferences {
  ...
  differences.terms = before.terms.map((beforeTerm, idx) => {
    ...
    termDifferences['duration'] = durationDiff(
      beforeTerm['duration'],
      after.terms?.[idx]?.['duration']
    );
    termDifferences['languages'] = languageDiff(
      beforeTerm['languages'],
      after.terms?.[idx]?.['languages']
    );
    termDifferences['exclusive'] = simpleDiff(
      beforeTerm['exclusive'],
      after.terms?.[idx]?.['exclusive']
    );
