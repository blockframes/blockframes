...
import { BucketTerm, Term, Negotiation } from '@blockframes/shared/model';
...
const createControl = (term: Partial<BucketTerm | Term> = {}) => {
  return 'id' in term ? { ...createBucketTermControl(term), id: new FormControl(term.id) } : createBucketTermControl(term);
};
