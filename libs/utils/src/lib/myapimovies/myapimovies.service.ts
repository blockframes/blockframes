  ...
  private async genres(imdbId: string): Promise<Genre[]> {
    try {
      ...
      const genres = data.map(d => {
        ...
        this.logs.error.push(`Skipped unknown genre ${d.genre} for ${imdbId}`);
      }).filter(d => !!d);
  ...
  private async countries(imdbId: string): Promise<Territory[]> {
    try {
      ...
      const countries = data.map(d => {
        ...
        this.logs.error.push(`Skipped unknown territory ${d.country} for ${imdbId}`);
      }).filter(d => !!d);
  ...
  private async languages(imdbId: string): Promise<Language[]> {
    try {
      ...
      const languages = data.map(d => {
        ...
        this.logs.error.push(`Skipped unknown language ${d.language} for ${imdbId}`);
      }).filter(d => !!d);
