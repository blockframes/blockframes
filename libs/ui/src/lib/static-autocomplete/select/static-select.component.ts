...
export class StaticSelectComponent {
  ...
  @Input() set scope(value: Scope) {
    this.option = value;
    this.staticValue = Object.keys(staticModel[value])
  };
  ...
  @Input() set withoutValues(toFilterValue: any[]) {
    this.staticValue = Object.keys(staticModel[this.option]).filter(scopeValue => !toFilterValue.includes(scopeValue))
  }
