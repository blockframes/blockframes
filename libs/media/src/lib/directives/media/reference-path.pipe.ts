...
const referencesPaths = {
    movie: {
        banner: (docId: string) => `movies/${docId}/main.banner`,
        poster: (docId: string) => `movies/${docId}/main.poster`,
        still: (docId: string, elementId: string) => `movies/${docId}/promotionalElements.still_photo.${elementId}`,
    },
    profile: {
