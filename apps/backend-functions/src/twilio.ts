...
import { PublicUser, EventDocument, Meeting } from '@blockframes/shared/model';
 ...
 */
export const getTwilioAccessToken = async (data: RequestAccessToken, context: CallableContext): Promise<ErrorResultResponse> => {
  if (!context.auth) {
...
export const twilioWebhook = async (req: Request, res: Response) => {
  ...
  try {
    if (
      ...
      req.body.AccountSid !== twilioAccountSid || // we are interested only by those 2 events
      (req.body.StatusCallbackEvent !== 'participant-disconnected' && req.body.StatusCallbackEvent !== 'room-ended')
    ) {
