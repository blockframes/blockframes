...
const queryMovieAnalytics = `
  SELECT
    ...
    REGEXP_EXTRACT(value.string_value, '.*/marketplace/title/([^/]+)/main') as movieIdPage
  FROM
    \`${bigQueryAnalyticsTable}*\`,
    UNNEST(event_params) AS params
  WHERE
    (
      (event_name = @pageView AND key = 'page_path' AND REGEXP_EXTRACT(value.string_value, '.*/marketplace/title/([^/]+)/main') in UNNEST(@movieIds))
      OR
