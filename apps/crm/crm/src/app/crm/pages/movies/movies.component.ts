  ...
  public exportTable(movies: CrmMovie[]) {
    try {
      ...
      const exportedRows = movies.map((m) => ({
        ...
        'audience targets': m.audience?.targets?.length ? toLabel(m.audience.targets, 'socialGoals') : '',
        ...
        'format quality': m.formatQuality ? toLabel(m.formatQuality, 'movieFormatQuality') : '',
