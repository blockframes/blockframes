// Angular
import { OnInit, OnDestroy, Component, ChangeDetectorRef, ChangeDetectionStrategy } from '@angular/core';
...
export class ListComponent implements OnDestroy, OnInit {
  ...
  async ngOnInit() {
    ...
    this.queries$ = combineLatest([
      this.contractService.valueChanges(ref =>
        ref.where('type', '==', 'mandate').where('buyerId', '==', centralOrgId.catalog).where('status', '==', 'accepted')
      ),
      this.contractService.valueChanges(ref => ref.where('type', '==', 'sale').where('status', '==', 'accepted')),
    ]).pipe(
    ...
    const { search, avails = {} } = decodeUrl(this.route);
    ...
    const searchSub = search$.pipe(skip(1)).subscribe(([search, avails]) =>
    ...
    this.movies$ = search$
      .pipe(
        ...
        debounceTime(300),
        switchMap(async ([_, availsValue, bucketValue, queries]) => [
          await this.searchForm.search(true),
          availsValue,
          bucketValue,
          queries,
        ])
      ...
      .pipe(
        map(
          ([movies, availsValue, bucketValue, { mandates, mandateTerms, sales, saleTerms }]: [
            SearchResponse<AlgoliaMovie>,
            AvailsFilter,
            Bucket,
            { mandates: Mandate[]; mandateTerms: Term[]; sales: Sale[]; saleTerms: Term[] }
          ]) => {
  ...
  async addAvail(title: AlgoliaMovie & { mandates: FullMandate[] }) {
    ...
    this.snackbar
      .open(`${title.title.international} was added to your Selection`, 'GO TO SELECTION', { duration: 4000 })
      .onAction()
