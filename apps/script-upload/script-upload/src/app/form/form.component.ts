...
import { ScriptHashContract, SCRIPT_TYPES, ScriptService } from '@blockframes/script';
...
export class FormComponent implements OnInit {
  ...
  public types = SCRIPT_TYPES;

  constructor(
    private service: ScriptService,
    private auth: AuthQuery,
    private snackBar: MatSnackBar,
    private contract: ScriptHashContract,
  ...
  public async uploaded(content: Uint8Array) {
    try {
      ...
      const { hash: txHash } = await this.contract.functions.addScript(ipHash);
