...
export class WaterfallComponent implements OnInit {
  ...
  private async populateIncomes() {
    ...
    const madman_au = await this.contractService.getValue('madman_au');
    ...
    const telepool_ger = await this.contractService.getValue('telepool_ger');
    ...
    const netflix_us = await this.contractService.getValue('netflix_us');
